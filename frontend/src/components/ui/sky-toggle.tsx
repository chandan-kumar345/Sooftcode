...
interface SwitchProps {
  className?: string;
  size?: string; // Optional custom toggle size, e.g. "30px", "24px"
}

const Switch: React.FC<SwitchProps> = ({ className, size }) => {
  ...
  if (!mounted) {
    // Return a matching placeholder div to prevent layout shifts
    return (
      <div 
        className={className}
        style={{ 
  ...
  return (
    <StyledWrapper $toggleSize={size || '30px'} className={className}>
...
const StyledWrapper = styled.div<{ $toggleSize: string }>`
  ...
  .theme-switch__checkbox:checked + .theme-switch__container .theme-switch__sun {
    -webkit-transform: translateX(-100%) rotate(-120deg);
    -ms-transform: translateX(-100%) rotate(-120deg);
    transform: translateX(-100%) rotate(-120deg);
  }
