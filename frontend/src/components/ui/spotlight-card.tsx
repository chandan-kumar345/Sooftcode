...
export interface GlowCardProps {
  children: ReactNode;
  className?: string;
  glowColor?: 'blue' | 'purple' | 'green' | 'red' | 'orange' | 'cyan' | 'pink' | 'yellow' | 'violet';
  size?: 'sm' | 'md' | 'lg';
  width?: string | number;
...
const glowColorMap = {
  ...
  yellow: { base: 55, spread: 180 },
  violet: { base: 260, spread: 200 }
};
...
const GlowCard: React.FC<GlowCardProps> = ({ 
  ...
  const getInlineStyles = () => {
    const baseStyles: React.CSSProperties & Record<string, string | number> = {
      '--base': base,
      '--spread': spread,
      '--radius': '24', // matched with rounded-3xl
      '--border': '2',
      '--backdrop': 'var(--card)', // matching the theme's card color
