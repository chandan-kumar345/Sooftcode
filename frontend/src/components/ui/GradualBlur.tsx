...
const useResponsiveDimension = (responsive: boolean, config: any, key: 'height' | 'width') => {
  ...
  useEffect(() => {
    ...
    const calc = () => {
      ...
      if (w <= 480 && config[`mobile${upperKey}`])
        v = config[`mobile${upperKey}`];
      else if (w <= 768 && config[`tablet${upperKey}`])
        v = config[`tablet${upperKey}`];
      else if (w <= 1024 && config[`desktop${upperKey}`])
        v = config[`desktop${upperKey}`];
      setValue(v);
    };
