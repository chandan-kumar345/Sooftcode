    ...
    const onMove = (e: MouseEvent) => {
      ...
      for (const dot of dotsRef.current) {
        ...
        if (speed > speedTrigger && dist < proximity && !dot._inertiaApplied) {
          ...
          // Proximity-based falloff: closest dots shift more, outer dots shift minimally
          const falloff = Math.max(0, 1 - dist / proximity);
          const pushX = (dot.cx - pr.x) * 0.48 * falloff + vx * 0.0060 * falloff;
          const pushY = (dot.cy - pr.y) * 0.48 * falloff + vy * 0.0060 * falloff;
